...
import com.voltskiya.mechanics.food.util.merge.RottingMergeOld;
...
public class RottingHopper {
    ...
    private static void dealWithHopperMoveSecond(HopperDone hopperDone) {
        while (!hopperDone.secondWaveMergeEvents.isEmpty()) {
            ...
            if (Charts.furanceTypes.contains(destination.getType())) {
                ...
                if (sourceLocation.getBlockY() != destinationLocation.getBlockY()) {
                    ...
                    if (RottingMergeOld.pushItem(destination, event.getItem(), destinationContents[0], 0))
                        // then stop
                        return;
                } else {
                    // then put it in the 1th slot
                    if (RottingMergeOld.pushItem(destination, event.getItem(), destinationContents[1], 1))
                        // then stop
                        return;
                }
            } else if (destination.getType() == InventoryType.BREWING) {
                if (RottingMergeOld.pushItem(destination, event.getItem(), destination.getContents()[3], 3))
                    // then stop
                    return;
            } else {
                if (RottingMergeOld.pushItem(destination, event.getItem())) {
                    // then stop
                    hopperDone.clear();
                    return;
                }
    ...
    public static void addEvent(Location key, InventoryMoveItemEvent event) {
        synchronized (moveSyncObject) {
            HopperDone keyValue = hoppers.getOrDefault(key, null);
            if (keyValue == null) {
                hoppers.put(key, new HopperDone(event));
                VoltskiyaPlugin.get().scheduleSyncDelayedTask(() -> RottingHopper.dealWithHopperMove(key));
            } else {
    ...
    public static void addEvent(UUID key, InventoryMoveItemEvent event) {
        synchronized (moveMinecartSyncObject) {
            ...
            if (keyValue == null) {
                ...
                VoltskiyaPlugin.get().scheduleSyncDelayedTask(() -> RottingHopper.dealWithMinecartMove(key), 0);
            } else {
                if (keyValue.firstWaveCheckEvents.isEmpty())
                    // if there doesn't exist one already, do this
                    VoltskiyaPlugin.get().scheduleSyncDelayedTask(() -> RottingHopper.dealWithMinecartMove(key), 0);
